...
## Основные группы и подгруппы колледжа

В разделе колледжа основная группа содержит только название (например, `ПИ-4-25`).
Внутри создаются подгруппы по предметам: Fullstack, ИИ, английский и другие.
У каждой подгруппы свой ментор, расписание, уроки и табель. При добавлении студента
выберите основную группу и одну или несколько её подгрупп. Имя родителя необязательно;
телефон родителя остаётся обязательным для отправки отчётов.

После обновления выполните `python tabel_project/manage.py migrate`.
Старые предметные группы сохраняются в разделе «Без основной группы»: создайте
основную группу и привяжите к ней существующие подгруппы через «Изменить».
Миграция не переименовывает группы и не переносит уроки или оценки.

API основных групп: `/api/college-groups/` (заголовок `X-Organization-Type: college`).
Создание и изменение доступны администратору. У подгрупп `/api/groups/` появилось
поле `main_group`; студент получает `main_group` и `main_group_name` в ответе API.

### Текст отчёта колледжа для родителя

Отчёт содержит основную группу, имя студента, отчётный месяц, количество оценок
5/4/3/2 отдельно по каждому предмету и уникальные даты пропусков. Неотмеченные уроки
не считаются пропусками. Текст формируется на кыргызском в `report.message_text`
и передаётся в Dify также отдельным входом `message_text`.

Чтобы сообщение доставлялось именно в этом формате, в опубликованном workflow Dify
добавьте строковый вход `message_text` и используйте его как текст отправляемого
сообщения (либо возьмите `message_text` из уже существующего входа `report`).
Не пересоздавайте этот текст через LLM. Получатель — `recipient_phone`, который
берётся из `StudentProfile.parent_phone`. Настройки внешнего workflow не хранятся
в этом репозитории и изменяются отдельно.

Период остаётся календарным месяцем: пример с маем и июнем не объединяет месяцы
в одну рассылку. Правила последнего урока месяца и защиты от повторной успешной
